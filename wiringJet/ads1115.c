/*
 * wiringJet ads1115.c, copied from
 * http://wiringpi.com/
 * 
 * ads1115.c:
...
static int myAnalogRead(struct wiringJetNodeStruct *node, int pin)
{
	...
	wiringJetI2CWriteReg16(node->fd, 1, config);

	// Wait for the conversion to complete

	  for(;  ;)
	{
		result =  wiringJetI2CReadReg16(node->fd, 1);
	...
	result =  wiringJetI2CReadReg16(node->fd, 0);
	result = __bswap_16(result);
 ...
 *********************************************************************************
 */
static void myDigitalWrite(struct wiringJetNodeStruct *node, int pin, int data)
{
	int chan = pin - node->pinBase;
	chan &= 3;
 ...
 *********************************************************************************
 */
static void myAnalogWrite(struct wiringJetNodeStruct *node, int pin, int data)
{
	int chan = pin - node->pinBase;
	int reg;
	int16_t ndata;
	...
	wiringJetI2CWriteReg16(node->fd, reg, data);
}
 ...
 *********************************************************************************
 */
int ads1115Setup(const int bus, const int pinBase, int i2cAddr)
{
	struct wiringJetNodeStruct *node = wiringJetNewNode(pinBase, 8);
	if (!node)
	{
		LogFormatted(LogLevelWarn, "ads1115.c", "ads1115Setup", "Invalid pin base and size. Pins %d through %d are not available.", pinBase, pinBase + 7);
		return -1;
	}
	
	int fd;
	if ((fd = wiringJetI2CSetup(bus, i2cAddr)) < 0)
		return 0 ;
