/*************************************************************************
 * wiringJet pca9685.c, copied from:
 * 
 * pca9685.c
 *
 ...
 */
int pca9685Setup(const int bus, const int pinBase, const int i2cAddress, float freq)
{
	// Create a node with 16 pins [0..15] + [16] for all
	struct wiringJetNodeStruct *node = wiringJetNewNode(pinBase, PIN_ALL + 1);
	if(!node)
	{
	...
	LogFormatted(LogLevelInfo, "pca9685.c", "pca9685Setup", "Created PCA9685 on bus %d at address 0x%x. Pin base %d. File Descriptor (fd) %d", bus, i2cAddress, pinBase, fd);

	// Setup the chip. Enable auto-increment of registers.
	int settings = wiringJetI2CReadReg8(fd, PCA9685_MODE1) & 0x7F;
	int autoInc = settings | 0x20;

	wiringJetI2CWriteReg8(fd, PCA9685_MODE1, autoInc);
 ...
 * Frequency will be capped to range [40..1500] Hertz. Try 50 for servos.
 */
void pca9685PWMFreq(int fd, float freq)
{
	LogFormatted(LogLevelInfo, "pca9685.c", "pca9685PWMFreq", "Setting PWM frequency of fd %d to %.1lf Hz.", fd, freq);

	// Cap at min and max
	freq = (freq > 2500 ? 2500 : (freq < 40 ? 40 : freq));
	...
	int settings = wiringJetI2CReadReg8(fd, PCA9685_MODE1) & 0x7F; 	// Set restart bit to 0
	...
	wiringJetI2CWriteReg8(fd, PCA9685_MODE1, sleep);
	wiringJetI2CWriteReg8(fd, PCA9685_PRESCALE, prescale);
	wiringJetI2CWriteReg8(fd, PCA9685_MODE1, wake);
	
	// Now wait a millisecond until oscillator finished stabilizing and restart PWM.
	usleep(1000);
	wiringJetI2CWriteReg8(fd, PCA9685_MODE1, restart);
}
...
{
	LogFormatted(LogLevelInfo, "pca9685.c", "pca9685PWMReset", "PWM reset of fd %d.", fd); 
	wiringJetI2CWriteReg16(fd, LEDALL_ON_L, 0x0);
	wiringJetI2CWriteReg16(fd, LEDALL_ON_L + 2, 0x1000);
}
...
void pca9685PWMWrite(int fd, int pin, int on, int off)
{
	...
	wiringJetI2CWriteReg16(fd, reg, on  & 0x0FFF);
	wiringJetI2CWriteReg16(fd, reg + 2, off & 0x0FFF);
}
...
void pca9685PWMRead(int fd, int pin, int *on, int *off)
{
	...
	if (on)
		*on  = wiringJetI2CReadReg16(fd, reg);
	if (off)
		*off = wiringJetI2CReadReg16(fd, reg + 2);
}
...
void pca9685FullOn(int fd, int pin, int tf)
{
	...
	int state = wiringJetI2CReadReg8(fd, reg);

	// Set bit 4 to 1 or 0 accordingly
	state = tf ? (state | 0x10) : (state & 0xEF);

	wiringJetI2CWriteReg8(fd, reg, state);
